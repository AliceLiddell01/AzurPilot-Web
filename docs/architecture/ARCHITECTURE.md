...
- `module/webui/api.py` уже содержит частичные REST-, SSE-потоки и WebSocket-маршруты, включая статистику, потоки уведомлений/лаунчера, снимки экрана и управление устройством;
...
- самостоятельный режим устаревшего MCP может слушать все интерфейсы, а его Starlette-обёртка сейчас разрешает CORS с разрешением любых источников;
- текущие проверки «локальности» нельзя после появления обратного прокси обобщить до правила «прямой TCP-узел находится на петлевом интерфейсе, значит пользователь доверенный»;
...
Разрушительные и привилегированные действия не должны получать специфичные для адаптера обходные пути. Перезапуск эмулятора, остановка экземпляра, изменение конфигурации, изменение планировщика, управление устройством и будущие команды ИИ должны проходить через общий слой политик бэкенда, который способен обеспечивать:
...
- локальное UI-состояние может использовать небольшое отдельное хранилище только когда состояние URL, состояние компонента и серверное состояние не подходят;
- после появления фронтенда ожидаются модульные/компонентные тесты и браузерные дымовые тесты.

На момент исследовательского среза Этапа 0 React 19.2 является текущей линией React 19, а Vite 8 — текущей основной веткой Vite. Точные версии пакетов должны жить в будущем файле блокировки зависимостей, а не в архитектурной основе.
...
Механизм интеграции должен быть явным и версионированным. Git-подмодуль/поддерево не должно становиться неявной связью между репозиториями во время выполнения без отдельного ADR с доказанной необходимостью.
...
flowchart LR
    ...
    Edge -->|петлевой / локальный адрес| Backend[Бэкенд AzurPilot ASGI\n127.0.0.1:<порт-бэкенда>]
    Backend --> Static[Статический артефакт фронтенда]
    Backend --> Services[Слой приложения / сервисов]
```

Caddy — **единственная целевая публичная HTTP(S)-точка входа**. Бэкенд AzurPilot должен в целевой схеме слушать локальный петлевой адрес, например `127.0.0.1:<порт-бэкенда>`, и не требовать прямой публикации внутреннего порта приложения в Интернет.

Caddy выбран как целевая точка завершения TLS и обратного проксирования, потому что поддерживает автоматический HTTPS для корректно настроенных публичных имён и полноценный обратный прокси. Установка, DNS, проброс портов и машинно-зависимая конфигурация не входят в Этап 0.

Первичные источники:

- Автоматический HTTPS Caddy: <https://caddyserver.com/docs/automatic-https>
...
Браузерный трафик рабочей эксплуатации по умолчанию работает через один веб-источник. Публичная точка входа маршрутизирует UI, `/api/v1/...` и пути реального времени внутри одного веб-источника.

CORS с разрешением любых источников **не является** моделью безопасности рабочей эксплуатации. Среда разработки позже может использовать прокси Vite или намеренно ограниченные веб-источники среды разработки, но удобство разработки не должно переопределять границу доверия рабочей системы.

## 12. Граница доверия и правила обратного прокси

Обратный прокси меняет то, кого бэкенд видит как прямой TCP-узел. Поэтому:

> Запрос, пришедший по соединению через петлевой интерфейс от обратного прокси, не становится автоматически доверенным локальным запросом пользователя.
...
Позже они могут использовать общий поставщик идентификации или общий слой авторизации, но файл cookie браузерной сессии нельзя автоматически считать универсальным MCP-учётным данным. Каждому адаптеру нужен явный способ передачи учётных данных, модель авторизации, правила CSRF и ограничения веб-источника там, где они применимы, отзыв/срок действия и семантика аудита.
...
- содержит инструменты, которые напрямую обращаются к `ProcessManager`, `AzurLaneConfig`, `Device`, состоянию планировщика/конфигурации, логам, ADB и операциям эмулятора;
...
Возможности MCP следует моделировать по смыслу протокола, а не складывать всё в инструменты:

- **Ресурсы** — читаемый контекст/данные, если это соответствует их смыслу;
- **Промпты** — переиспользуемые шаблоны/сценарии запросов, где это полезно;
- **Инструменты** — вызываемые операции, особенно действия с параметрами или побочными эффектами.

Изменяющие состояние и разрушительные MCP-инструменты требуют серверных разрешений, валидации, аудита и участия человека там, где это необходимо. `/mcp` не должен публиковаться как отдельный неаутентифицированный порт бэкенда.
...
Идентификаторы и методы сервисов должны позволять явное указание экземпляра/цели там, где этого требует предметная модель. Это не разрешение проектировать многопользовательскую облачную систему SaaS; задача только в том, чтобы не зашить ненужное ограничение на один экземпляр в публичный контракт.
...
7. Перенести бизнес-действия MCP на общие сервисы до расширения привилегированного управления через ИИ.
...
- SaaS или многопользовательскую облачную архитектуру;
...
- генератор API-типов;
- заготовку React-приложения, точные версии пакетов или файл блокировки зависимостей;
...
| Точные версии фронтенд-пакетов | Должны находиться в файле блокировки зависимостей после создания основы приложения. |
...
| Означает ли прямой узел обратного прокси на петлевом интерфейсе доверенного локального пользователя? | Нет. |
